...
<div class="min-h-screen bg-cream font-jost">
  <div class="mx-auto max-w-xl px-6 py-16">
    ...
    <div class="mb-10"
      ...
      <h1 class="font-cormorant text-5xl font-normal text-brand-900">
        Cek Resi
      </h1>
      ...
      <div class="relative">
        <form action="/cek-resi" method="POST" class="flex flex-col gap-5">
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-light uppercase tracking-[3px] text-brand-700">
              Pilih Kurir
            </label>
            <select name="courier"
                    class="w-full border border-brand-200 bg-cream px-4 py-2.5 text-[13px]
                           font-light text-brand-900 focus:border-gold-400 focus:outline-none
                           transition-colors">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <input type="text" name="awb" required autocomplete="off"
                   ...
                   class="w-full border border-brand-200 bg-cream px-4 py-2.5 text-[13px]
                          font-light text-brand-900 placeholder:text-brand-300
          ...
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-light uppercase tracking-[3px] text-brand-700">
              ...
              <span class="ml-1 normal-case text-[9px] tracking-normal text-brand-500">(Opsional)</span>
            </label>
            <input type="text" name="phone"
                   ...
                   class="w-full border border-brand-200 bg-cream px-4 py-2.5 text-[13px]
                          font-light text-brand-900 placeholder:text-brand-300
                          focus:border-gold-400 focus:outline-none transition-colors">
            <p class="text-[10px] font-light text-brand-500">
          ...
          {{-- Submit --}}
          <button type="submit"
                  class="flex w-full items-center justify-center gap-3 bg-brand-800 py-3.5
    ...
    <div class="mt-6 border border-brand-200 bg-cream"
         data-aos="fade-up" data-aos-duration="500">

      {{-- Result header --}}
      <div class="border-b border-brand-200 bg-brand-50/60 px-5 py-4">
        ...
        <p class="font-cormorant text-2xl font-normal text-brand-900">
          {{ $result['data']['summary']['awb'] }}
        </p>
        <div class="mt-2 flex items-center gap-2">
          <span class="h-1.5 w-1.5 rounded-full bg-brand-400"></span>
          <span class="text-[11px] font-light uppercase tracking-[2px] text-brand-600">
      ...
      <div class="px-5 pb-2">
        <p class="text-[10px] font-light uppercase tracking-[3px] text-brand-700">
          Riwayat Perjalanan
        </p>
      </div>
        ...
        <div class="flex gap-4 {{ !$loop->last ? 'pb-4' : '' }}">
          ...
          <div class="flex flex-col items-center">
            <div class="h-2.5 w-2.5 shrink-0 border border-gold-400
                        ...
                        mt-1.5"></div>
            @if(!$loop->last)
              <div class="mt-1 w-px flex-1 bg-brand-200"></div>
            @endif
          </div>

          {{-- Content --}}
          <div class="min-w-0 pb-1">
            <p class="text-[10px] font-light uppercase tracking-[2px] text-brand-500 mb-0.5">
              {{ $log['date'] }}
            </p>
            <p class="text-[12px] font-light leading-relaxed text-brand-800">
              {{ $log['desc'] }}
            </p>
          </div>
