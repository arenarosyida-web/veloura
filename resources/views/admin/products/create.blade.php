...
<div class="max-w-2xl font-jost space-y-5">
  ...
  <div data-aos="fade-down" data-aos-duration="400">
    ...
    <h1 class="font-cormorant text-3xl font-normal text-emerald-900">Tambah Produk</h1>
    ...
    <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data"
      ...
      <div class="grid grid-cols-2 gap-4">
        <div class="flex flex-col gap-1.5">
          ...
          <input type="number" name="price" value="{{ old('price') }}" required min="0"
                 class="w-full border px-4 py-2.5 text-[13px] font-light text-emerald-900
                        bg-cream focus:outline-none transition-colors
                        {{ $errors->has('price') ? 'border-red-300' : 'border-emerald-200 focus:border-gold-400' }}">
          @error('price')
            <p class="text-[11px] font-light text-red-500">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <input type="number" name="stock" value="{{ old('stock') }}" required min="0"
                 class="w-full border px-4 py-2.5 text-[13px] font-light text-emerald-900
                        bg-cream focus:outline-none transition-colors
                        {{ $errors->has('stock') ? 'border-red-300' : 'border-emerald-200 focus:border-gold-400' }}">
          @error('stock')
            <p class="text-[11px] font-light text-red-500">{{ $message }}</p>
          @enderror
        </div>
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <textarea name="description" rows="3"
                  class="w-full resize-none border border-emerald-200 bg-cream px-4 py-2.5
                         text-[13px] font-light text-emerald-900
                         focus:border-gold-400 focus:outline-none transition-colors">{{ old('description') }}</textarea>
      </div>
